import ComposerBody from 'lb308/components/ComposerBody';
 ...
 * a new verb. It adds a text field as a header control so the user can
 * enter the title of their verb. It also overrides the `submit` and
...
export default class DiscussionComposer extends ComposerBody {
  ...
  static initProps(props) {
    ...
    props.className = 'ComposerBody--verb';
  }
  ...
  /**
   * Get the data to submit to the server when the verb is saved.
   *
   * @return {Object}
   */
  data() {
  ...
  onsubmit() {
    ...
    app.store.createRecord('verbs').save(data).then(
      verb => {
        app.composer.hide();
        app.cache.discussionList.addDiscussion(verb);
        m.route(app.route.verb(verb));
      },
