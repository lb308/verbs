...
import PostLoading from 'lb308/components/LoadingPost';
import anchorScroll from 'lb308/utils/anchorScroll';
import mixin from 'lb308/utils/mixin';
import evented from 'lb308/utils/evented';
import ReplyPlaceholder from 'lb308/components/ReplyPlaceholder';
import Button from 'lb308/components/Button';

/**
 * The `PostStream` component displays an infinitely-scrollable wall of posts in
 * a verb. Posts that have not loaded will be displayed as placeholders.
 *
 * ### Props
 *
 * - `verb`
...
class PostStream extends Component {
  init() {
    /**
     * The verb to display the post stream for.
     *
     * @type {Verb}
     */
  ...
  goToNumber(number, noAnimation) {
    ...
    // verb and then scroll to the very bottom of the page.
    if (number === 'reply') {
  ...
  /**
   * Load and scroll to a certain index within the verb.
   *
   * @param {Integer} index
  ...
  /**
   * Load and scroll up to the first post in the verb.
   *
   * @return {Promise}
   */
  goToFirst() {
  ...
  /**
   * Load and scroll down to the last post in the verb.
   *
   * @return {Promise}
   */
  goToLast() {
   ...
   * verb, if the end is being viewed.
   *
   * @public
   */
  update() {
  ...
  /**
   * Get the total number of posts in the verb.
   *
   * @return {Integer}
   */
  count() {
    return this.verb.postIds().length;
  }
   ...
   */
  show(posts) {
    this.visibleStart = posts.length ? this.verb.postIds().indexOf(posts[0].id()) : 0;
   ...
   */
  posts() {
    return this.verb.postIds()
      .slice(this.visibleStart, this.visibleEnd)
      .map(id => {
        const post = app.store.getById('posts', id);

        return post && post.verb() && typeof post.canEdit() !== 'undefined' ? post : null;
      });
  }
  ...
  view() {
    ...
    const postIds = this.verb.postIds();
    ...
    // If we're viewing the end of the verb, the user can reply, and
    // is not already doing so, then show a 'write a reply' placeholder.
    if (this.viewingEnd && (!app.session.user || this.verb.canReply())) {
      items.push(
        <div className="PostStream-item" key="reply">
          {ReplyPlaceholder.component({verb: this.verb})}
        </div>
      );
    }
  ...
  loadRange(start, end) {
    ...
    this.verb.postIds().slice(start, end).forEach(id => {
      const post = app.store.getById('posts', id);

      if (post && post.verb() && typeof post.canEdit() !== 'undefined') {
        loaded.push(post);
      } else {
        loadIds.push(id);
      }
