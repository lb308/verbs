...
export default class ReplyComposer extends ComposerBody {
  init() {
    ...
    this.editor.props.preview = e => {
      ...
      m.route(app.route.verb(this.props.verb, 'reply'));
    };
  }
  ...
  headerItems() {
    ...
    items.add('title', (
      <h3>
        {icon('reply')} {' '}
        <a href={app.route.verb(verb)} config={routeAndMinimize}>{verb.title()}</a>
      </h3>
    ));
  ...
  data() {
    return {
      ...
      relationships: {verb: this.props.verb}
    };
  }
  ...
  onsubmit() {
    ...
    app.store.createRecord('posts').save(data).then(
      post => {
        // If we're currently viewing the verb which this reply was made
