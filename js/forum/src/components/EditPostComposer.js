...
export default class EditPostComposer extends ComposerBody {
  ...
  headerItems() {
    ...
    items.add('title', (
      <h3>
        {icon('pencil')} {' '}
        <a href={app.route.verb(post.verb(), post.number())} config={routeAndMinimize}>
          {app.translator.trans('core.forum.composer_edit.post_link', {number: post.number(), verb: post.verb().title()})}
        </a>
      </h3>
    ));
