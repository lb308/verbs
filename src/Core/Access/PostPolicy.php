...
class PostPolicy extends AbstractPolicy
{
    ...
    {
        if ($actor->can($ability.'Posts', $post->verb)) {
            return true;
        }
    }
    ...
    public function scopePostVisibility(ScopePostVisibility $event)
    {
        ...
        $event->query->where(function ($query) use ($event) {
            ...
            $this->events->fire(
                new ScopePrivatePostVisibility($event->verb, $query, $event->actor)
            );
        });

        // When fetching a verb's posts: if the user doesn't have permission
        // to moderate the verb, then they can't see posts that have been
        // hidden by someone other than themself.
        if ($event->actor->cannot('editPosts', $event->verb)) {
            $event->query->where(function ($query) use ($event) {
    ...
    public function edit(User $actor, Post $post)
    {
        ...
        // the verb which it's in, or if they are the author and the post
        ...
        if ($post->user_id == $actor->id && (! $post->hide_time || $post->hide_user_id == $actor->id)) {
            ...
            if ($allowEditing === '-1'
                || ($allowEditing === 'reply' && $post->number >= $post->verb->last_post_number)
