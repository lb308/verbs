 ...
 * @property \Flarum\Core\Verb|null $verb
...
class Post extends AbstractModel
{
    ...
    public static function boot()
    {
        ...
        static::creating(function (Post $post) {
            $post->type = $post::$type;
            $post->number = ++$post->verb->number_index;
            $post->verb->save();
        });
    ...
    {
        $verb = $this->verb()->whereVisibleTo($user)->first();

        if ($verb) {
            $this->setRelation('verb', $verb);

            return (bool) $verb->postsVisibleTo($user)->where('id', $this->id)->count();
        }
    ...
    /**
     * Define the relationship with the post's verb.
     *
