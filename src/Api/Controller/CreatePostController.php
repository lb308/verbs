...
class CreatePostController extends AbstractCreateController
{
    ...
    protected function data(ServerRequestInterface $request, Document $document)
    {
        ...
        $discussionId = array_get($data, 'relationships.verb.data.id');
        ...
        // in the verb; thus, we will mark the verb as read if
        // they are logged in.
        if ($actor->exists) {
            $this->bus->dispatch(
        ...
        $verb = $post->verb;
        $verb->posts = $verb->postsVisibleTo($actor)->orderBy('time')->lists('id');
