...
class ListPostsController extends AbstractCollectionController
{
    ...
    protected function extractOffset(ServerRequestInterface $request)
    {
        ...
        if (($near = array_get($queryParams, 'page.near')) > 1) {
            if (count($filter) > 1 || ! isset($filter['verb']) || $sort) {
                throw new InvalidParameterException(
                    'You can only use page[near] with filter[verb] and the default sort order'
                );
            }

            $offset = $this->posts->getIndexForNumber($filter['verb'], $near, $actor);
    ...
    {
        if ($discussionId = array_get($filter, 'verb')) {
