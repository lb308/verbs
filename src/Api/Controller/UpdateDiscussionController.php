...
class UpdateDiscussionController extends AbstractResourceController
{
    ...
    protected function data(ServerRequestInterface $request, Document $document)
    {
        ...
        if ($posts = $verb->getModifiedPosts()) {
            $posts = (new Collection($posts))->load('verb', 'user');
            $discussionPosts = $verb->postsVisibleTo($actor)->orderBy('time')->lists('id')->all();
            ...
            $verb->setRelation('posts', $discussionPosts);

            $this->include = array_merge($this->include, ['posts', 'posts.verb', 'posts.user']);
        }
