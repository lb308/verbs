...
class DiscussionController extends WebAppController
{
    ...
    protected function getView(Request $request)
    {
        ...
        $url = function ($newQueryParams) use ($queryParams, $document) {
            ...
            return $this->url->toRoute('verb', ['id' => $document->data->id]).
        ...
        foreach ($document->included as $resource) {
            if ($resource->type === 'posts' && isset($resource->relationships->verb) && isset($resource->attributes->contentHtml)) {
                $posts[] = $resource;
            }
        }
        ...
        $view->content = app('view')->make('flarum.forum::verb', compact('document', 'page', 'getResource', 'posts', 'url'));

        return $view;
    }

    /**
     * Get the result of an API request to show a verb.
     *
     * @param User $actor
     * @param array $params
     * @return object
